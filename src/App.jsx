...
function App() {
  ...
  // --- INICIALIZAÇÃO ---
  useEffect(() => {
    document.documentElement.classList.add('dark'); // Força modo escuro
  }, []);
  ...
  const fetchDados = async () => {
      if(!supabase) {
          // DADOS DE EXEMPLO (FALLBACK) SE NÃO TIVER CHAVES
          console.warn("Supabase não configurado. Usando dados locais de exemplo.");
          setTaxasFrete([{id:1, nome:'Centro', valor:5}]);
          setProdutos([{id:1, nome:'Dog Simples', preco:15, categoria:'Lanches', tipo:'principal', estoque: 100}]);
          setLoading(false);
          return;
      }
  ...
  const abrirMonteDog = () => { 
      setFormPedido(getFormPedidoInicial()); 
      // Seleciona defaults se existirem
  ...
  const imprimir = (p) => {
    const subtotal = calcularTotal(p.itens, 0, 0); // Calcula subtotal puro
  ...
  return (
    <div className="flex h-screen font-sans overflow-hidden dark bg-zinc-950 text-zinc-100">
      ...
      <main className="flex-1 overflow-y-auto relative flex flex-col bg-zinc-950">
        <header className="md:hidden bg-zinc-900 text-white p-4 flex justify-between items-center shadow-md sticky top-0 z-10 border-b border-zinc-800"><div className="flex items-center gap-2 font-extrabold italic text-xl">{logoUrl ? <img src={logoUrl} alt="Logo" className="w-8 h-8 rounded object-cover"/> : <Utensils className="text-yellow-500"/>} BEST DOG</div>{pedidosPendentes.length > 0 && <span className="text-xs bg-red-600 text-yellow-100 font-bold px-3 py-1 rounded-full shadow-sm">{pedidosPendentes.length}</span>}</header>
        <div className="p-4 md:p-8 pb-24 md:pb-8 max-w-7xl mx-auto w-full">
            ...
            {abaAtiva === 'dashboard' && (
              <div className="space-y-4">
                 ...
                 <button onClick={abrirMonteDog} className="w-full py-4 bg-indigo-600 text-white font-extrabold rounded-xl shadow-md hover:bg-indigo-500 flex justify-center items-center gap-3 border-b-4 border-indigo-800 active:border-b-0 active:translate-y-1 transition-all"><Edit3 size={22} className="text-yellow-300"/> MONTE SEU DOG</button>
                 <div className="mt-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">{pedidosPendentes.map(p => (<div key={p.id} className="bg-zinc-900 rounded-lg shadow-sm border border-zinc-800 flex flex-col relative overflow-hidden hover:border-zinc-700 transition-all"><div className={`h-1 w-full ${p.status === 'Saiu para Entrega' ? 'bg-orange-500' : 'bg-red-500'}`}></div><div className="p-3 flex flex-col gap-1"><div className="flex justify-between items-center"><span className="font-black text-white text-lg">#{p.id?.toString().slice(-4)}</span><div className="flex items-center gap-2"><button onClick={() => abrirNoMaps(p.cliente?.endereco || '')} className="text-blue-400 bg-blue-900/20 p-1 rounded hover:bg-blue-900/40"><Map size={14}/></button><Badge status={p.status}/></div></div><div className="text-sm font-bold text-zinc-300 truncate">{p.cliente?.nome}</div><div className="text-xs text-zinc-500 mb-2 flex items-center gap-1"><Utensils size={10}/> {(p.itens||[]).length} itens • {p.hora}</div><div className="text-xs text-zinc-400 bg-zinc-950 border-l-4 border-zinc-700 pl-2 py-1 mb-2 font-medium">{(p.itens||[]).map(i => `${i.qtd}x ${i.nome}`).join(', ').substring(0, 60)}{p.itens?.length > 2 ? '...' : ''}</div><div className="flex justify-between items-end mt-auto pt-2 border-t border-zinc-800"><div><div className="text-[10px] uppercase text-zinc-500 font-bold">{p.pagamento}</div><div className="font-black text-red-500 text-lg">{formatarMoeda(p.total)}</div></div><div className="flex gap-1"><button onClick={() => imprimir(p)} className="p-1.5 bg-zinc-800 text-zinc-300 rounded hover:bg-zinc-700"><Printer size={14}/></button><button onClick={() => enviarZap(p)} className="p-1.5 bg-green-900/30 text-green-400 rounded hover:bg-green-900/50"><MessageCircle size={14}/></button><button onClick={() => editarPedido(p)} className="p-1.5 bg-amber-900/30 text-amber-500 rounded hover:bg-amber-900/50"><Pencil size={14}/></button></div></div></div><button onClick={() => avançarStatus(p.id)} className={`w-full py-2 text-xs font-bold text-white flex items-center justify-center gap-1 ${p.status === 'Pendente' ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}`}>{p.status === 'Pendente' ? <><ArrowRight size={14}/> MANDAR ENTREGA</> : <><CheckCircle size={14}/> CONCLUIR</>}</button></div>))}</div>
                 </div>
              </div>
            )}
            ...
            {/* ABA PEDIDOS */}
            {abaAtiva === 'pedidos' && (
              <div className="space-y-4"><div className="flex justify-between items-center mb-2"><h2 className="text-xl font-bold text-zinc-100">Lista Detalhada</h2></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{pedidosPendentes.map(p => (<div key={p.id} className="bg-zinc-900 rounded-2xl shadow-sm border border-zinc-800 overflow-hidden p-5"><div className="flex justify-between"><strong className="text-white">#{p.id?.toString().slice(-4)}</strong><Badge status={p.status}/></div><div className="mt-2"><p className="font-bold text-gray-200">{p.cliente?.nome}</p><p className="text-xs text-gray-500">{p.cliente?.endereco}</p></div><div className="bg-zinc-950 p-2 rounded text-sm mt-2 text-gray-300">{(p.itens||[]).map((i, idx) => (<div key={idx}>{i.qtd}x {i.nome}</div>))}</div><div className="flex justify-between items-center mt-3 pt-3 border-t border-zinc-800"><span className="font-black text-red-400">{formatarMoeda(p.total)}</span><div className="flex gap-2"><button onClick={() => editarPedido(p)} className="text-xs bg-zinc-800 px-2 py-1 rounded text-white">Editar</button><button onClick={() => cancelarPedido(p.id)} className="text-xs bg-red-900 text-red-300 px-2 py-1 rounded">Cancelar</button></div></div></div>))}</div></div>
            )}
            ...
            {abaAtiva === 'config' && (
                <div className="space-y-6">
                    ...
                    <Card className="p-6 border-zinc-800 bg-zinc-900">
                        <h3 className="font-bold text-lg mb-4 text-zinc-100 flex items-center gap-2"><ImageIcon size={20}/> Logo</h3>
                        <input placeholder="URL da Logo" className="w-full border border-zinc-700 bg-zinc-950 text-zinc-200 rounded-lg p-2 text-sm" value={logoUrl} onChange={e => setLogoUrl(e.target.value)}/>
                    </Card>
                    <Card className="p-6 border-zinc-800 bg-zinc-900"><h3 className="font-bold text-lg mb-4 text-zinc-100">Áudio</h3><button onClick={tocarSom} className="px-4 py-2 bg-amber-900 text-amber-200 rounded-lg font-bold flex items-center gap-2 hover:bg-amber-800"><Play size={18} /> Testar Som</button></Card>
